...
- [ ] Run with_skill eval using `benchmark/run_eval.sh`:
  ```
  bash benchmark/run_eval.sh <project-dir> "verify the acceptance criteria for task 003" benchmark/iteration-1/eval-10-verify-task/with_skill/outputs --allowedTools "Bash,taskmd:verify-task"
  ```
- [ ] Run without_skill baseline using `benchmark/run_eval.sh` with taskmd blocked:
  ```
  PATH="$SHADOW_DIR:$PATH" bash benchmark/run_eval.sh <project-dir> "verify the acceptance criteria for task 003" benchmark/iteration-1/eval-10-verify-task/without_skill/outputs --allowedTools "Bash"
  ```
