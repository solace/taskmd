...
Run the verify-task skill **with and without** the taskmd skill loaded in an isolated project, then compare quality, accuracy, token usage, and latency.
...
- [ ] Run **without_skill** baseline: `claude -p "verify the acceptance criteria for task 003"` (no skill loaded)
- [ ] Save without_skill output to `benchmark/iteration-1/eval-10-verify-task/without_skill/outputs/result.md`
- [ ] Run **with_skill** variant: `claude -p "verify the acceptance criteria for task 003" --allowedTools "taskmd:*"` (skill loaded)
- [ ] Save with_skill output to `benchmark/iteration-1/eval-10-verify-task/with_skill/outputs/result.md`
