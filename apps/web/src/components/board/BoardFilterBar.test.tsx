...
describe("BoardFilterBar", () => {
  ...
  it("calls onEffortsChange with all efforts when clicking all button in Effort row", () => {
    ...
    fireEvent.click(screen.getByText("Filters"));
    const allButtons = screen.getAllByText("all");
    // Effort row: after Priority (0) and Effort (1) — depends on groupBy
    // With groupBy="status", rows are: Priority, Effort, Type. So Effort all = index 1
  ...
  it("calls onTypesChange with all types when clicking all button in Type row", () => {
    ...
    fireEvent.click(screen.getByText("Filters"));
    const allButtons = screen.getAllByText("all");
    // Type row: after Priority (0), Effort (1), Type (2). So Type all = index 2
  ...
  it("calls onPrioritiesChange with all priorities when clicking all button in Priority row", () => {
    ...
    fireEvent.click(screen.getByText("Filters"));
    const allButtons = screen.getAllByText("all");
    // Priority row is the first row when groupBy="status"
    fireEvent.click(allButtons[0]);
